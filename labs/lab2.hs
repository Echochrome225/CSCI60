...
-- For example, palindrome [5, 2, 7] = [5, 2, 7, 7, 2, 5]

palindrome :: [Int] -> [Int]
palindrome xs = concat [xs, (reverse xs)]
...
list1 = [x | x <- [100..200], mod x 3 /= 0, mod x 5 /= 0, mod x 7 /= 0]
list2 = [x * x - 2 * x + 10| x <- [30,31..50]]
...
-- so that it correctly computes the greatest common divisor of a and b,
...
extGCD a 0 = (a,1,0)
extGCD 0 b = (b,0,1)
extGCD a b = let (d, m, n) = extGCD (b `mod` a) a in (d, n - (b `div` a) * m, m)